...
LogReader::LogReader (string const& logFile,
	string const& loginsFile) : m_logFile (logFile),
...
void LogReader::match (void) {
	...
	for (list<LogRec>::iterator oit =
		m_logouts.begin (); oit != m_logouts.end ();
		++oit)
		// 在登入日志记录集中查找匹配记录...
		for (list<LogRec>::iterator iit =
			m_logins.begin (); iit != m_logins.end ();
			++iit)
			if (! strcmp (oit->logname,
				iit->logname) &&
				! strcmp (oit->logip,
				iit->logip) &&
				oit->pid == iit->pid) {
...
void LogReader::saveLoginsFile (void)
	...
	if (! m_logins.empty ()) {
		...
		while (! m_logins.empty ()) {
			ofs.write ((char const*)&m_logins.front (),
				sizeof (LogRec));
			m_logins.pop_front ();
		}
		ofs.close ();
